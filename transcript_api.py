...
class TranscriptApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/api/people":
                ...
                return
            if parsed.path == "/api/person-repairs":
                self.write_json(
                    identity_review_workflow.IdentityReviewWorkflow(
                        self.store_root,
                        gold_root=(
        ...
        except identity_review_workflow.StaleDirectoryHypothesisReview as exc:
            self.write_error(HTTPStatus.CONFLICT, str(exc))
        except ValueError as exc:
    ...
    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        try:
            if parsed.path.startswith("/api/person-repairs/"):
                parts = [unquote(part) for part in parsed.path.split("/") if part]
                if len(parts) == 3:
            ...
            if parsed.path.startswith("/api/organization-repairs/"):
                ...
                if len(parts) == 3:
                    ...
                    return
            if parsed.path.startswith("/api/directory-hypotheses/"):
                parts = [unquote(part) for part in parsed.path.split("/") if part]
                if len(parts) == 4 and parts[3] == "reviews":
                    ...
                    self.write_json(
                        ...
                        ).record_directory_hypothesis_review(body),
                        status=HTTPStatus.CREATED,
                    )
                    return
