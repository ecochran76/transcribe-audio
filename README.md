...
Optional Google Calendar integration can rename files and embed event metadata. The shared calendar lookup uses an explicit provider order and, when `--use-calendar` is set, also resolves user-scoped provenance sources from `~/.local/state/transcribe-audio/provenance.config.json`. ffmpeg support enables subtitle embedding.
...
- `--provenance-config` / `--provenance-profile`: Override the shared user-scoped provenance config or active profile used for calendar, contact, and context-source resolution.
...
Calendar metadata, participant identity, context workbench, route/context CLI, the watcher, and the web API share one user-scoped config:

```text
~/.local/state/transcribe-audio/provenance.config.json
```

The tracked `provenance.config.json.sample` documents the schema without real secrets. Runtime config supports `gog`, `gws`, `msgcli`, multiple `odollo` tenant profiles, and arbitrary `ical_calendar` feeds. Private iCalendar URLs may be stored as user-scoped literal `url` values or `url_ref` environment references; API responses and run manifests redact them.

Inspect and mutate the config through the shared CLI:

```bash
python provenance_config.py show
python provenance_config.py doctor
python provenance_config.py init-sample
python provenance_config.py preview-update --update-file update.json
...
```bash
python assembly_transcribe.py meeting.m4a --use-calendar --provenance-profile default
...
- `calendar.provenance_profile`: optional shared provenance profile; private feeds and shared-calendar source lists should live in `~/.local/state/transcribe-audio/provenance.config.json`, not in watcher config
...
Use the shared provenance config for GWS/Odollo context sources:

```bash
python route_transcript.py "meeting Transcript.transcript.json" "meeting Transcript.readout.json" \
  --configured-provenance --provenance-profile default
```

This resolves the `context_workbench` workflow source list from `provenance.config.json` while preserving the existing explicit `--gws-provenance` and `--odollo-provenance` flags for one-off diagnostics.
...
is intentionally disabled, so differences in existing project instructions,
files, or settings remain operator-visible rather than an execution error.
...
The API exposes `/api/library`, `/api/conversations`, `/api/review-queue`, `/api/intelligence/providers`, `/api/intelligence/smokes`, `/api/intelligence/smoke-jobs`, `/api/intelligence/config`, `/api/automation/config`, `/api/provenance/config`, `/api/provenance/config/doctor`, `/api/provenance/config/preview`, `/api/provenance/config/apply`, prepared App Intelligence run ledgers under `/api/intelligence/runs`, App Intelligence session-start preflight/control-plane start/model-turn preflight, prompt-packet review, registered run artifact reads, replay manifests, send-preflight, gated model-turn send, turn-status capture, structured-decision validation, ledger-only structured-decision apply, local human-review annotation/resolution, fork-branch preflight, and rollback preflight under `/api/intelligence/runs/<run_id>/...`, first-pass summary batch actions under `/api/review-queue/first-pass-summaries/`, `/api/search`, `/api/documents/<id>`, `/api/documents/<id>/related`, `/api/documents/<id>/context`, and range-capable `/api/blobs/<blob_id>` routes over the user-scoped store and runtime state. `GET /api/conversations` groups transcript, first-pass readout, and contextual readout artifacts by source transcript, supports `kind` and `query` filters for server-backed Library search, and returns workflow/media metadata without returning artifact text or reading artifact files. `GET /api/provenance/config` returns the redacted shared provenance config plus resolved calendar/contact settings; preview/apply endpoints deep-merge local config updates and apply requires `approval_token=APPLY_PROVENANCE_CONFIG_UPDATE`. `GET /api/automation/config` returns user-scoped stage automation policy for ingestion, transcription, initial summary, speaker identity, context collection, and final readout; preview/apply endpoints mutate only `~/.local/state/transcribe-audio/automation.config.json` or `TRANSCRIPTS_AUTOMATION_CONFIG`, apply requires `approval_token=APPLY_AUTOMATION_CONFIG_UPDATE`, and config inspection never runs workflow stages. `GET /api/conversations/<document_id>` returns one conversation workspace payload with selected/source/summary/contextual documents, extracted participants, and linked media from the store. `GET /api/documents/<id>/related` resolves readout `source_artifact_path` links back to stored source transcripts and lists readouts/contextual readouts derived from a selected transcript. `GET /api/intelligence/smokes` reports latest smoke evidence metadata without reading screenshot or artifact contents. `POST /api/intelligence/smoke-jobs` queues only allowlisted smoke/cleanup commands with explicit approval tokens and writes job records under the user-scoped state directory; it does not accept arbitrary shell input. `GET /api/intelligence/smoke-jobs/<job_id>/tail` reads only bounded stdout/stderr tails for a recorded smoke job and confines output paths to the smoke-job directory. `GET /api/intelligence/runs/<run_id>` includes structured decision history, prompt/status artifact paths, and event-log preflight artifact records for read-only replay inspection in the console, `GET /api/intelligence/runs/<run_id>/replay-manifest` returns ordered artifact metadata without reading content, and `GET /api/intelligence/runs/<run_id>/artifacts?path=<path>` serves only artifacts already recorded in that run ledger or event log. `intelligence_config.py` centralizes task-level provider/model/timeout routing and resolves from built-in defaults, `~/.local/state/transcribe-audio/intelligence.config.json`, `TRANSCRIPTS_INTELLIGENCE_CONFIG`, per-task environment overrides, and explicit CLI/API overrides. `automation_config.py` centralizes workflow stage automation policy and is surfaced in the React console's Settings tab. `provenance_config.py` centralizes user-scoped calendar, contact, and context-source routing and is surfaced in the React console's Provenance tab. The review queue includes route reviews, App Intelligence `ask_for_human_review` decisions, filename conflicts, and first-pass summary counts. The first-pass prepare endpoint writes a dry-run manifest only; submit requires that prepared manifest and an approval token. The selected-conversation one-click initial summary endpoint prepares that scoped manifest and submits it in one reviewed call using the same `SUBMIT_FIRST_PASS_SUMMARY_BATCH` approval token. The App Intelligence prepare endpoint creates only a local run ledger; it does not start app-server sessions or provider work. The session-start preflight endpoint validates readiness and can optionally append a preflight event, but it never starts app-server sessions or provider work. The session-start endpoint starts only the managed Codex app-server control-plane daemon for a prepared ledger and records events before any model turn. The model-turn preflight endpoint writes reviewable prompt artifacts only and does not send prompts; `GET /api/intelligence/runs/<run_id>/prompt-packets/<packet_id>` reads packet JSON/text for inspection and surfaces the future `SEND_APP_SERVER_MODEL_TURN` token without using it. `POST /api/intelligence/runs/<run_id>/prompt-packets/<packet_id>/send-preflight` requires that token and validates packet eligibility while still returning `will_send_prompt=false` and `will_write_event=false`. `POST /api/intelligence/runs/<run_id>/prompt-packets/<packet_id>/send` uses the same token to start one Codex app-server turn, records thread/turn ids and captured app-server events, and still returns `will_execute_downstream_action=false`. `POST /api/intelligence/runs/<run_id>/turn-status` requires `CAPTURE_MODEL_TURN_STATUS`, captures output to a run artifact, and still returns `will_execute_structured_decision=false`. `POST /api/intelligence/runs/<run_id>/structured-decision/validate` requires `VALIDATE_STRUCTURED_DECISION`, validates and records accepted/rejected decision metadata, and still returns `will_execute_host_action=false`. `POST /api/intelligence/runs/<run_id>/structured-decisions/<decision_id>/apply` requires `APPLY_STRUCTURED_DECISION`, currently records validated `continue_current_branch`, `stop`, and `ask_for_human_review` decisions in the local ledger, and still returns no external action, write-bearing action, fork, or rollback. `continue_current_branch` leaves the run open as `current_branch_continued`. `POST /api/intelligence/runs/<run_id>/structured-decisions/<decision_id>/human-review` requires `RECORD_HUMAN_REVIEW_DECISION` and only annotates, resolves, or reopens the local ledger review state. `POST /api/intelligence/runs/<run_id>/structured-decisions/<decision_id>/fork-preflight` requires `PREVIEW_FORK_BRANCHES` and writes a preview artifact without creating threads, modifying branches, or running provider work. `POST /api/intelligence/runs/<run_id>/structured-decisions/<decision_id>/rollback-preflight` requires `PREVIEW_ROLLBACK` and writes a preview artifact without modifying branches, reverting artifacts, creating threads, or running provider work. When `frontend/dist/` exists, the same server serves the built React console at `/`. Transcript ingestion registers existing source recordings as copied blobs under `~/.transcripts/blobs/` and links them to documents through SQLite pointers, so the UI can play recordings without streaming arbitrary original filesystem paths. If the service environment cannot find `codex` on `PATH`, set `TRANSCRIPTS_CODEX_BIN` or pass `--codex-bin /absolute/path/to/codex` so `codex-app-server` readiness can be reported. See `docs/dev/transcript-review-api.md` for the endpoint contract.
...
python automation_config.py show
python automation_config.py preview-update --stage initial_summary --enabled true --mode one_click
python automation_config.py apply-update --stage initial_summary --enabled true --mode one_click --approval-token APPLY_AUTOMATION_CONFIG_UPDATE
```
...
4. Additional flags:
   ...
   - `--calendar-provenance-calendar-id` for additional shared calendars that
     ...
     event used for filenames. Repeat the flag for a one-off run, or store the
     calendar IDs in the shared provenance config for normal CLI/watcher use.
   - `--calendar-provenance-ical-url` for private iCalendar feed URLs such as
     Zoho exports that should contribute overlap/attendee provenance. Repeat
     the flag for a one-off run, or store the feed as an `ical_calendar` source
     in the shared user-scoped provenance config. Keep private feed URLs in
     user-scoped runtime config, not committed docs or watcher config.
   ...
   - `--provenance-config` and `--provenance-profile` to select the shared
     provenance registry used by direct CLI mode.
...
When accessible or configured provenance calendars contain overlapping events for the same recording window, the transcript sidecar stores `event.matching_calendars`. Each entry includes the calendar ID, calendar summary, access role when available, event ID/summary, event start/end, overlap seconds, coverage, and attendee emails when the calendar provider returns them. Private iCalendar feeds are recorded only as stable hashed calendar IDs such as `ical:<hash>`. This is intended for readout/routing and deterministic speaker-identity context; it does not change the primary event selected for filenames.
