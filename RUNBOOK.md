...
synthetic temporary stores. Knowledge schema v7 adds exact run/stage history,
source-scoped adapter receipts, nonbinding hypotheses, visible Evidence
Strength pillars, deterministic re-score lineage, and source-disjoint
calibration history.

Authority:
`docs/dev/plans/0072-2026-08-16-correction-first-speaker-contact-learning.md`,
`docs/correction-first-evidence-supervisor.md`, Note 0058, and the A0 adapter
and supervisor contracts.

Evidence:

- `ConversationKnowledgeStore` migrates v6 to v7 additively and rolls v7 back
  to v6 without changing A1 identity, A2 transcript, or A3 biometric history.
- `identity_evidence_supervisor.py` validates the A0 processing-run and adapter
  artifacts. Runs retain actual original filenames, source hashes, as-of time,
  policy/model/rubric/profile versions, inputs, budgets, failures, and zero
  effect counts.
- Stage events advance through the frozen supervisor sequence only. Exact
  replays return unchanged; drift, skipping, and post-completion transitions
  fail closed.
- Adapter exchanges bind provider kind, profile, account, tenant, capability,
  request, run, and result. Cumulative records, characters, calls, and latency
  cannot exceed the run budget. One retry follows only a named transient
  failure, and every result reports zero provider writes.
- Successful observations remain present when another capability is partial or
  unavailable. Calendar association, purpose, and participant artifacts remain
  explicit hypotheses and assign no identity or speech.
- Calendar, person-link, contextual-speaker, and acoustic scores retain their
  factors, evidence IDs, and independence groups. Material contradiction or a
  duplicated independence group caps combined Evidence Strength at 49 and
  requires review.
- Re-scores cite the exact predecessor, rubric, and model. Calibrated
  Likelihood remains unavailable below 30 source-disjoint outcomes; eligible
  snapshots include rate, sample size, Wilson interval, evaluation version,
  and input watermark but grant no promotion authority.
- A4 used no provider, private corpus, historical conversation, live store,
  accepted identity/profile effect, queue publication, deployment, worker,
  external effect, delegation, or Graphiti write.

Validation:

- Focused supervisor/store/profile/evidence/biometric validation passes 43
  tests; seven are A4-specific.
- `.venv/bin/python -m pytest -q` passes 1,174 tests in 122.01 seconds.
- Python compilation, the active planning audit, changed-document relative
  link checks, A4 JSON parsing, and `git diff --check` pass. The active audit
  reports `ok: true`, zero problems, and Plan 0072 as `OPEN`. The goal-only
  audit returns `ok: true` but `applicable: false` because the corresponding
  durable policy module isn't present in this checkout.
- CodeGraph reports 357 indexed files, 10,477 nodes, and 36,210 edges with no
  pending-file or worktree-mismatch warning.
- A transcription smoke is not applicable because A4 changes no transcription
  provider, watcher, media decoding, or DOCX/TXT output path.

Next:

- Execute Plan 0072 A5 on redacted fixture data: add queue and People
  projections, stale-safe decision/effect-preview APIs, and original-filename-
  bearing Identity Review and People views. Preserve Authelia as the only
  authentication layer and keep provider, private-shadow, accepted-effect,
  public publication, and deployment gates closed.

## Turn 394: Close Plan 0072 A3 biometric custody (2026-08-16)

Summary: Closed the non-live A3 biometric-custody packet on generated byte
strings and synthetic temporary stores. Knowledge schema v6 adds governed
sample inventory, soft anonymous clusters, reviewed profile families and
versions, deterministic rebuild receipts, previewed exclusions and deletion,
and minimal audit tombstones.

Authority:
`docs/dev/plans/0072-2026-08-16-correction-first-speaker-contact-learning.md`,
`docs/correction-first-biometric-custody.md`, Note 0058, and the A0 biometric
contracts.

Evidence:

- `ConversationKnowledgeStore` migrates v5 to v6 additively and rolls v6 back
  to v5 without changing A1 identity or A2 transcript history.
- `biometric_custody_ledger.py` separates portable metadata from hash-bound
  test-private bytes. It rejects missing, symlinked, or broadly accessible
  private roots and enforces `0600` object permissions.
- Unreviewed samples remain person-unbound. Reviewed bindings require explicit
  review and consent authorities; profile allowlists also require eligible,
  included samples for the same person.
- Anonymous cluster versions preserve ranked soft memberships and evidence.
  Confirming one reviewed anchor records score changes and requeues only
  materially changed related unreviewed samples; it never bulk-assigns an
  identity.
- Profile families preserve condition-specific variants. Profile versions are
  pending until explicit activation, retain predecessors, support explicit
  supersession and rollback, and record byte-equal or drifted rebuild results.
- Exclusion and deletion previews cover sample, cluster, profile, recording,
  and person scopes. Stale previews fail before mutation. Deletion quarantines
  private bytes across the transaction, removes them after commit, and records
  future-backup exclusion plus scheduled historical-backup expiry.
- Redacted fixture replay contains synthetic labels and generated non-voice
  bytes only. Portable readbacks and tombstones expose no private path or
  private object identifier.
- A3 used no real voice, private corpus, historical conversation, live store,
  provider call/write, external benchmark, named enrollment, dashboard,
  deployment, worker, external effect, delegation, or Graphiti write.

Validation:

- Focused custody/store/profile/evidence validation passes 36 tests.
- `.venv/bin/python -m pytest -q` passes 1,166 tests in 117.51 seconds.
- Python compilation, the active planning audit, changed-document relative
  link checks, A3 JSON parsing, and `git diff --check` pass. The active audit
  reports `ok: true`, zero problems, and Plan 0072 as `OPEN`. The goal-only
  audit returns `ok: true` but `applicable: false` because the corresponding
  durable policy module isn't present in this checkout.
- CodeGraph reports 355 indexed files, 10,402 nodes, and 35,862 edges with no
  pending-file or worktree-mismatch warning.
- A transcription smoke is not applicable because A3 changes no transcription
  provider, watcher, media decoding, or DOCX/TXT output path.

Next:

- Execute Plan 0072 A4 on synthetic and redacted evidence only: compose the
  capability-scoped evidence supervisor, run ledger, independent pillar
  scores, calibration history, budgets, and partial-failure isolation with
  zero provider writes and zero accepted identity/profile effects.
...
- This is review-rework cycle 2 of 2. Sessions `c9c4d5d5fbd0` and
  `03268b59db56` are superseded for decision work.

Evidence:

- Implementation completed at `af0f70b`. Focused tests passed 8/8; the full
  suite passed 934 tests twice after final publish-scope adjustment.
- Immutable replay bound worksheet `0afd218a...`, manifest `5748e662...`, and
  receipt content `227f1e09...`. The publishable directory is exactly one HTML
  file plus 10 WAVs totaling 8,319,480 bytes; no manifest, receipt, full
  recording, or transcript is published.
- Authenticated Previews session `11aabed660d2`, artifact `3afb4a96364a`, is
  the sole active review surface. Browser proof found 10 audio controls and 10
  blank decisions; every WAV returned 200 `audio/x-wav`, `readyState=4`, no
  error, and a finite 18.690-30.779 second duration. Previews served a reviewed
  Range request as a complete 200 response without range headers; this is
  nonblocking for the bounded clip sizes. Page errors and artifact-scoped POST
  requests were zero; session feedback was empty.

Checkpoint: `OPEN/P2-direct-audio-republish -> OPEN/P2-human-gate`; progress
`outcome_progress`; next: operator listens and returns the exact complete
13-line decision block. No decision is inferred from approval or silence.
