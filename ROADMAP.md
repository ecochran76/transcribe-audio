...
The conversation modal now includes a re-transcription dry-run preflight that resolves source blob, selected backend, planned output paths, command preview, and explicit no-queue/no-run/no-write safety flags before any future queueing action.
