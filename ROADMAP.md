...
The conversation modal now includes a re-transcription dry-run preflight that resolves source blob, selected backend, planned output paths, command preview, and explicit no-queue/no-run/no-write safety flags. It can write a reviewed queue manifest under `~/.local/state/transcribe-audio/retranscription-jobs/` after `QUEUE_RETRANSCRIPTION_JOB`, but that queue step still does not start a speech backend or write transcript outputs.
