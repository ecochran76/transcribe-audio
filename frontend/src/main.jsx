...
function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [healthPayload, libraryPayload] = await Promise.all([
    ...
    <main className="app-shell">
      ...
      >
          ...
          <div className="pane-content">
            ...
            <div className="filter-card">
              ...
              {REVIEW_BUCKETS.map((bucket) => (
                <button key={bucket.label} type="button">
                  {bucket.label}
        ...
        <section className="center-pane">
          <div className="view-heading">
            ...
            <div className="summary-strip">
              ...
              <span>{REVIEW_BUCKETS.reduce((total, item) => total + item.count, 0)} open reviews</span>
            </div>
          </div>
        ...
        <aside className="right-pane">
          ...
          <Inspector item={selected} activeNav={activeNav} />
        </aside>
      </section>
    </main>
  );
...
function ReviewQueue() {
  return (
    <div className="review-grid">
      {REVIEW_BUCKETS.map((bucket) => (
        <article className={`review-card ${bucket.status}`} key={bucket.label}>
...
function Inspector({ item, activeNav }) {
    ...
    return (
      <div className="inspector-content">
        ...
        <dl>
          ...
          <dd>Closed as no-mutation decisions.</dd>
          <dt>Next queue</dt>
          <dd>Legacy enrichment needs first-pass summaries.</dd>
        </dl>
      </div>
    );
  }
