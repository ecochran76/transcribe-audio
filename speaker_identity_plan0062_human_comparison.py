...
def _validated_sources(
    ...
    if p4.get("schema_version") == review.MANIFEST_SCHEMA:
        ...
        packet = p4
    packet_core = {key: value for key, value in packet.items() if key != "content_sha256"}
    if (
        packet.get("schema_version") != review.PACKET_SCHEMA
        or packet.get("status") != "awaiting_literal_human_review"
        or packet.get("p3_content_sha256") != p3_content_sha256
    ...
    if selected == "unresolved":
        ...
        if source == "canonical_context_proposal":
            ...
            decision["binding_status"] = (
                "acoustic_subject_requires_reviewed_person_binding"
            )
    else:
...
def freeze_human_comparison(
    ...
    *,
    ...
    submission = parse_human_submission(
        answer_text, p3_manifest=p3_manifest, p4_source=p4_source
    )
    comparison = recompute_comparison(p3_manifest, p4_source, submission)
    ...
    independently_recomputed = (
        recompute_comparison(p3_manifest, p4_source, submission) == comparison
    )
...
def replay_human_comparison(
    *,
    ...
    expected = recompute_comparison(p3_manifest, p4_source, submission)
    if (
